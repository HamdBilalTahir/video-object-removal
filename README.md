...
## 📂 Download (GPU Required)

If you are running the full MiniMax-Remover diffusion pipeline on GPU, you must download the main model weights:

```shell
huggingface-cli download zibojia/minimax-remover --include vae transformer scheduler --local-dir .
```

> **Note:** The Gradio Demo below uses a CPU-friendly LaMa fallback for object removal and does not require these heavy diffusion weights.

---

## ⚡ Quick Start

### Run Gradio Demo (CPU-Friendly Fallback)

The easiest way to interact with the tracking and removal pipeline using the LaMa fallback is via the Gradio Web UI.
...
```bash
cd gradio_demo
source ../minimaxvenv/bin/activate
gradio test.py
```
This will automatically download the necessary models (`facebook/sam2-hiera-large`, `big-lama`) on the first run and host a local web server for you to use.

---

### Minimal Code Example (GPU Required)
