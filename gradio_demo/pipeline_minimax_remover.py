...
class Minimax_Remover_Pipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        batch_size = 1
        transformer_dtype = torch.float16 if device.type == "cuda" else torch.float32
        ...
        masks = self.resize(masks, height, width).to(device).to(transformer_dtype)
        masks[masks>0] = 1
        images = rearrange(images, "f h w c -> c f h w")
        images = self.resize(images[None,...], height, width).to(device).to(transformer_dtype)
        ...
        latents_mean = (
                ...
                .to(self.vae.device, transformer_dtype)
            )

        latents_std =  1.0 / torch.tensor(self.vae.config.latents_std).view(1, self.vae.config.z_dim, 1, 1, 1).to(
                self.vae.device, transformer_dtype
            )

        with torch.no_grad():
            masked_latents = self.vae.encode(masked_images.to(transformer_dtype)).latent_dist.mode()
            masks_latents = self.vae.encode((2*masks.to(transformer_dtype)-1.0)).latent_dist.mode()
        ...
        latents = latents.to(transformer_dtype) / latents_std + latents_mean
